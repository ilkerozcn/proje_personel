...
namespace proje_personel
{
    public partial class Form2 : Form
    {
        ...
        private void Form1_Load_1(object sender, EventArgs e)
        {
            ...
            this.doviz_cinsiTableAdapter.Fill(this.kullanici_projeDataSet.doviz_cinsi);
        ...
        private void button7_Click(object sender, EventArgs e)
        {
            ...
            using (SqlConnection connection = new SqlConnection("Data Source=DESKTOP-PPM9TPL;Initial Catalog=kullanici_proje;Integrated Security=True"))
            {
                ...
                string mustersorgu = "Select * FROM musteri_bilgileri WHERE tc_no = '" + maskedTextBox20.Text + "'";
                ...
                using (SqlDataReader reader = command.ExecuteReader())
                {
                    if (reader.Read())
                    {
                        ...
                        maskedTextBox10.Text = anneKizlikSoyad;
                        comboBox6.SelectedValue = egitimDurumu;
                        comboBox8.SelectedValue = sube;
                        label38.Text = "Ad Soyad: " + ad + " " + soyad + "\n Müşterı No: " + musteriNo;
        ...
        private void button8_Click(object sender, EventArgs e)
        {
            ...
            string connectionString = "Data Source=DESKTOP-PPM9TPL;Initial Catalog=kullanici_proje;Integrated Security=True";

            // Müşteri numarası
            Int64 targetCustomerNo = hesapMusteriNo; // Değiştirmeniz gereken müşteri numarası
            int hesapSayisi = 0;
            ...
            using (SqlConnection connection = new SqlConnection(connectionString))
            {
                try
                {
                    ...
                    // SQL sorgusu
                    string query = "SELECT COUNT(*) FROM musteri_hesap_bilgileri WHERE musteri_no = @CustomerNo";

                    // Sorguyu çalıştırmak için komut oluştur
                    using (SqlCommand command = new SqlCommand(query, connection))
                    {
                        // Parametre ekleyerek SQL sorgusunu güvenli hale getir
                        command.Parameters.AddWithValue("@CustomerNo", targetCustomerNo);

                        // Sorguyu çalıştır ve sonucu al
                        hesapSayisi = (int)command.ExecuteScalar();

                        // Sonucu ekrana yazdır
                        Console.WriteLine($"Müşteri numarası {targetCustomerNo} ile eşleşen kayıt sayısı: {hesapSayisi}");
                    }
                }
                catch (Exception ex)
                {
            ...
            komut1.Parameters.AddWithValue("@musteri_no", hesapMusteriNo);
            komut1.Parameters.AddWithValue("@hesap_no", hesap_no);
            komut1.Parameters.AddWithValue("@doviz_cinsi", comboBox9.SelectedValue);
            komut1.Parameters.AddWithValue("@bakiye", textBox1.Text);
            komut1.Parameters.AddWithValue("@sube_kodu", comboBox8.SelectedValue);
            komut1.Parameters.AddWithValue("@durum_kodu", 1);
            komut1.Parameters.AddWithValue("@hesap_rumuzu", textBox2.Text);
            komut1.Parameters.AddWithValue("@hesap_acilis_tarihi", DateTime.Now.ToString("MM - dd - yyyy"));
